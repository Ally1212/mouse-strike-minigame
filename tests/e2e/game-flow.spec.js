...
test("攻击、技能、变身、僚机和双触点操作互不冲突", async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page);
  await openGame(page);
  const hangar = await getLayout(page, "hangar");
  await clickRect(page, hangar.start);
  ...
  await clickRect(page, combatLayout.actions.form);
  expect(await page.evaluate(() => globalThis.__mouseStrikeMiniGame.state.combat.toolModeIndex)).toBe(1);
  ...
  await page.waitForFunction(() => globalThis.__mouseStrikeMiniGame.app.renderer.uiLayer.texts
  ...
  await clickRect(page, combatLayout.actions.wingman);
  ...
  const multiTouch = await page.evaluate(async () => {
    ...
    const action = {
      identifier: 72,
      clientX: layout.actions.form.x + layout.actions.form.width / 2,
      clientY: layout.actions.form.y + layout.actions.form.height / 2,
    };
...
test("战斗 HUD 与暂停弹窗在窄屏保持完整", async ({ page }) => {
  ...
  const labels = await page.evaluate(() => globalThis.__mouseStrikeMiniGame.app.renderer.uiLayer.texts
    .filter((slot) => slot.sprite.visible)
    .map((slot) => slot.key.split("|")[0]));
  expect(labels).toEqual(expect.arrayContaining(["中国 / 歼-20", "攻击", "技能", "变身", "僚机"]));
